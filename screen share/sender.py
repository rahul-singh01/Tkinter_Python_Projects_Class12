...
reciever = AudioReceiver(ip , 8888)
...
def camera():
    cam = CameraClient(text_ip.get(1.0,'end-1c'),7777)
    t3 = threading.Thread(target=cam.start_stream)
    t3.start()

def screen():
    sc = ScreenShareClient(text_ip.get(1.0,'end-1c'),7777)
    t4 = threading.Thread(target=sc.start_stream)
    t4.start()

def audio():
    au = AudioSender(text_ip.get(1.0,'end-1c'),6666)
